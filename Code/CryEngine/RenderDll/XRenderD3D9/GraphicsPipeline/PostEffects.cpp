...
void CSharpeningPass::Execute()
{
	...
	// -- Now allow full range (including negative, 0, >1)
	const f32 fSharpenAmount = CRenderer::CV_r_Sharpening;
	const f32 fTextureSharpenAmount = CRenderer::CV_r_TextureSharpening;
	const f32 fVibranceAmount = CRenderer::CV_r_Vibrance;

	// If you want to allow pAmount to override *all* (advanced: otherwise remove this)
	// const f32 fSharpenAmount        = pAmount->GetParam();
	// const f32 fTextureSharpenAmount = pAmount->GetParam();
	// const f32 fVibranceAmount       = pAmount->GetParam();
	...
	Vec4 pParams[3] = {
		Vec4(CRenderer::CV_r_ChromaticAberration, 0, 0, fSharpenAmount),
		Vec4(CRenderer::CV_r_TextureSharpening, 0, 0, fTextureSharpenAmount),
		Vec4(CRenderer::CV_r_Vibrance, 0, 0, fVibranceAmount)
	};

	static CCryNameR pParamName("psParams");
	pass.SetConstantArray(pParamName, pParams, 3);
...
void CVibrancePass::Init(CPostEffectContext* p)
{
	m_pContext = p;
}
...
void CVibrancePass::Execute()
{
	...
	CRY_ASSERT(pAmount != nullptr);
	CRY_ASSERT(pVibrance != nullptr); // Check vibrance param exists

	if (!pAmount || !pVibrance)
	{
		return;
	}

	PROFILE_LABEL_SCOPE("VIBRANCE");
	...
	const f32 fVibranceAmount = max(pAmount->GetParam(), CRenderer::CV_r_Vibrance);

	auto& pass = m_passVibrance;

	if (pass.IsDirty(pDstTex->GetID(), pSrcTex->GetID(), pipelineResources.m_pTexDisplayTargetScaled[0]->GetID()))
	{
		static CCryNameTSCRC techName("AdjustVibrance"); // Make sure this matches your shader
		pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
		pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
		pass.SetTechnique(CShaderMan::s_shPostEffects, techName, 0);
		pass.SetState(GS_NODEPTHTEST);
		...
		pass.SetTexture(0, pSrcTex); // <-- Set input texture
		pass.SetSampler(0, EDefaultSamplerStates::PointClamp); // <-- Set sampler
	...
	// Insert vibrance value into the .z component of psParams[0]
	Vec4 pParams = Vec4(CRenderer::CV_r_Vibrance, fVibranceAmount, 0.0f, fVibranceAmount);
